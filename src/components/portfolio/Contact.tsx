...
const Contact = () => {
  ...
  return (
    <section id="contact" className="relative py-32 border-t border-border overflow-hidden">
      ...
      <div className="container relative">
        <div className="max-w-4xl">
          ...
          <form
            ...
            className={`mt-16 grid gap-5 p-8 rounded-3xl border border-border bg-card/40 backdrop-blur-sm reveal reveal-scale ${formReveal.visible ? "is-visible" : ""}`}
            noValidate
          >
          ...
          <div ref={cards.ref} className={`mt-16 grid sm:grid-cols-3 gap-4 reveal ${cards.visible ? "is-visible" : ""}`}>
            {[
              ...
              <a
                ...
                className="group p-6 rounded-2xl border border-border bg-card/40 backdrop-blur-sm hover:border-primary hover:-translate-y-1 hover:shadow-[0_10px_30px_-15px_hsl(var(--primary)/0.5)] transition-all duration-300 shine"
                style={{ transitionDelay: `${i * 80}ms` }}
              >
                <c.icon className="size-5 text-muted-foreground mb-3 transition-all duration-300 group-hover:text-primary group-hover:scale-110" />
      ...
      <footer className="container mt-32 pt-8 border-t border-border flex flex-wrap items-center justify-between gap-4 text-sm text-muted-foreground">
        <div className="font-mono">© 2026 Lucas Tabacchi · Fullstack Developer orientado a producto</div>
        <div className="font-mono text-xs">v1.0 — diseñado y construido a mano</div>
      </footer>
    </section>
  );
};
